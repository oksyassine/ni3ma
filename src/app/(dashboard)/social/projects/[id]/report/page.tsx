    ...
    <div className="space-y-4 print:space-y-2">
      ...
      <div className="bg-white text-black rounded-lg p-8 print:p-0 max-w-4xl mx-auto">
        <div className="text-center border-b-2 border-primary pb-3 mb-6">
          <h1 className="text-3xl font-bold text-primary">جمعية النعمة - مكناس</h1>
          <p className="text-sm">تقرير {PROJECT_KIND_LABELS[p.kind]}</p>
        </div>
        ...
        <table className="w-full text-sm mb-4">
          <tbody>
            <tr className="border-b"><td className="font-bold py-1 w-40">النوع</td><td>{PROJECT_KIND_LABELS[p.kind]}</td></tr>
            {p.targetAudience && <tr className="border-b"><td className="font-bold py-1">الفئة المستهدفة</td><td>{p.targetAudience}</td></tr>}
            {p.location && <tr className="border-b"><td className="font-bold py-1">المكان</td><td>{p.location}</td></tr>}
            {p.partners && <tr className="border-b"><td className="font-bold py-1">الشركاء</td><td>{p.partners}</td></tr>}
            {p.startDate && <tr className="border-b"><td className="font-bold py-1">تاريخ البداية</td><td>{p.startDate.toISOString().slice(0, 10)}</td></tr>}
            {p.endDate && <tr className="border-b"><td className="font-bold py-1">تاريخ النهاية</td><td>{p.endDate.toISOString().slice(0, 10)}</td></tr>}
          </tbody>
        </table>
        ...
        <div className="grid grid-cols-4 gap-3 mb-4">
          ...
          <div className="border rounded p-3 text-center">
            <p className="text-xs text-gray-600">جمع التبرعات</p>
            <p className="text-2xl font-bold text-green-700">{(cashTotal + inKindTotal).toFixed(0)} د.م</p>
          </div>
          <div className="border rounded p-3 text-center">
          ...
          <div className="border rounded p-3 text-center">
            <p className="text-xs text-gray-600">ساعات التطوع</p>
        ...
        {p.evaluationScore && (
          <div className="border rounded p-4 mb-4 bg-yellow-50">
            <p className="text-sm font-bold mb-1">التقييم العام</p>
        ...
        {p.evaluationReport && (
          <div className="mb-4">
            <h3 className="text-lg font-bold border-b pb-1 mb-2">ملخص التقرير</h3>
        ...
        {p.evaluationLessons && (
          <div className="mb-4">
            <h3 className="text-lg font-bold border-b pb-1 mb-2">الدروس المستفادة</h3>
        ...
        {p.evaluationRecommend && (
          <div className="mb-4">
            <h3 className="text-lg font-bold border-b pb-1 mb-2">التوصيات</h3>
        ...
        {p.inKindDonations.length > 0 && (
          <div className="mb-4">
            <h3 className="text-lg font-bold border-b pb-1 mb-2">تفصيل التبرعات العينية</h3>
            <ul className="text-sm list-disc ms-5">
              {p.inKindDonations.map((d, i) => (
                <li key={i}>{d.itemName} — {Number(d.quantity).toString()} {d.unit ?? ""}{d.estimatedValue ? ` (≈ ${Number(d.estimatedValue).toFixed(0)} د.م)` : ""}</li>
              ))}
            </ul>
          </div>
        )}
        ...
        <div className="mt-8 text-xs text-gray-600 border-t pt-3 flex justify-between">
          <span>أعدّ المشروع: {p.creator?.fullName ?? "—"}</span>
          {p.evaluator && <span>قيّمه: {p.evaluator.fullName} · {p.evaluatedAt?.toISOString().slice(0, 10)}</span>}
        </div>
      </div>
    </div>
  );
